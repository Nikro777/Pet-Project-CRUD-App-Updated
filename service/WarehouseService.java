...
public class WarehouseService {
    ...
    public void deleteProduct(Long id) {
        try {
            ...
            throw new RuntimeException("Товар с ID: " + id + " не существует");
        }
    }

    public Warehouse moveProductsFromTo(Long id, Integer toPlace){
        Warehouse warehouse = warehouseRepository.findById(id).orElseThrow(
                () -> new RuntimeException("Товар с ID: " + id + " не найден"));

        if (warehouseRepository.existsByWarehousePlace(toPlace)){
    ...
    public Warehouse getDecreaseQuantity(Long id, Integer decrease) {
        Warehouse warehouse = warehouseRepository.findById(id).orElseThrow(
                () -> new RuntimeException("Товар с ID: " + id + " не найден"));
         if (warehouse.getQuantity() < decrease){
